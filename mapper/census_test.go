...
func TestCleanDimensionsLabel(t *testing.T) {
	...
	Convey("Given simple page data", t, func() {
		...
		Convey("and filter dimension labels that include category counts", func() {
			...
			Convey("when we build a dataset landing page", func() {
				page := CreateCensusFilterOutputsPage(req, pageModel, datasetModel, getTestVersionOneDetails(), "", false, []dataset.Version{getTestVersionOneDetails()}, 1, "/a/version/1", "", []string{}, 50, false, true, filter.Model{Downloads: getTestFilterDownloads([]string{"xlsx"})}, filterDimensions, serviceMessage, emergencyBanner, true, population.GetDimensionsResponse{}, cantabular.GetBlockedAreaCountResult{}, population.GetPopulationTypeResponse{})
