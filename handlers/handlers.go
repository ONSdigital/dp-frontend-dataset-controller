...
// Datasetpage will load a zebedee dataset page
func DatasetReferenceTablePage(zc ZebedeeClient, dc DatasetClient, rend RenderClient, cfg config.Config, apiRouterVersion string) http.HandlerFunc {
	return handlers.ControllerHandler(func(w http.ResponseWriter, req *http.Request, lang, collectionID, userAccessToken string) {
		datasetReferenceTablePage(w, req, zc, dc, rend, cfg, collectionID, lang, userAccessToken, apiRouterVersion)
	})
}
...
func datasetReferenceTablePage(w http.ResponseWriter, req *http.Request, zc ZebedeeClient, dc DatasetClient, rend RenderClient, cfg config.Config, collectionID, lang, userAccessToken string, apiRouterVersion string) {
	path := req.URL.Path
	ctx := req.Context()
	...
	m := mapper.CreateReferenceTablePage(ctx, req, ds, dlp, bc, versions, lang, apiRouterVersion)
	...
	templateHTML, err := rend.Do("reference-tables", templateJSON)
	if err != nil {
		setStatusCode(req, w, err)
		return
	}
